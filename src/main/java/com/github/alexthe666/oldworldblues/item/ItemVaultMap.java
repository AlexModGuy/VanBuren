...
public class ItemVaultMap extends ItemMapBase {
    ...
    public static ItemStack setupNewMap(World worldIn, double worldX, double worldZ, byte scale, boolean trackingPosition, boolean unlimitedTracking) {
        ItemStack itemstack = new ItemStack(OWBItems.FILLED_VAULT_MAP, 1, 0);
        ...
        mapdata.markDirty();
        VaultData data = OWBWorldData.get(worldIn).getNearestVault(new BlockPos(worldX, 0, worldZ), 10000);
        mapdata.addTargetDecoration(itemstack, data.getEnterence(), "vault__" + UUID.randomUUID(), MapDecoration.Type.MONUMENT);
        return itemstack;
    }
    ...
    public static void renderBiomePreviewMap(World worldIn, ItemStack map) {
        if (map.getItem() instanceof ItemVaultMap) {
            MapData mapdata = ((ItemVaultMap) map.getItem()).getMapData(map, worldIn);
